  ...
  var _loop = function _loop(fieldName) {
    ...
    if (Array.isArray(fieldValue.$all) && schema.fields[fieldName].type === 'Array') {
      (function () {
        ...
        var allowNull = false;
        values.push(fieldName);
        fieldValue.$all.forEach(function (listElem, listIndex) {
          if (listElem === null) {
            allowNull = true;
          } else {
            values.push(listElem);
  ...
  _createClass(PostgresStorageAdapter, [{
    ...
    value: function setClassLevelPermissions(className, CLPs) {
      ...
      return this._ensureSchemaCollectionExists().then(function () {
        var values = [className, 'schema', 'classLevelPermissions', CLPs];
    ...
    value: function deleteAllClasses() {
      ...
      return this._client.any('SELECT * FROM "_SCHEMA"').then(function (results) {
        ...
        var classes = ['_SCHEMA', '_PushStatus', '_Hooks', '_GlobalConfig'].concat(_toConsumableArray(results.map(function (result) {
    ...
    value: function findOneAndUpdate(className, schema, query, update) {
      ...
      return this._client.any(qs, values).then(function (val) {
        return val[0];
    ...
    value: function upsertOneObject(className, schema, query, update) {
      ...
      return this.createObject(className, schema, update).catch(function (err) {
    ...
    value: function find(className, schema, query, _ref3) {
      ...
      debug('find', className, query, { skip: skip, limit: limit, sort: sort });
      ...
      var qs = 'SELECT * FROM $1:name ' + wherePattern + ' ' + sortPattern + ' ' + limitPattern + ' ' + skipPattern;
      debug(qs, values);
    ...
    value: function performInitialization(_ref4) {
      ...
      return Promise.all(promises).then(function () {
        return Promise.all([_this9._client.any(json_object_set_key).catch(function (err) {
          console.error(err);
        }), _this9._client.any(array_add).catch(function (err) {
          console.error(err);
        }), _this9._client.any(array_add_unique).catch(function (err) {
          console.error(err);
        }), _this9._client.any(array_remove).catch(function (err) {
          console.error(err);
        })]);
      }).then(function () {
