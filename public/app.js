...
var App = {
    ...
    updateStage: function (data) {
        ...
        if (data.word.length > 0) {
            ...
            App.updateImage(data.img);
            App.createLetters(data.word);
        }
    },
    resetStage: function () {
        App.current_title = [];
        ...
        App.clearList(results);
        var letters = document.getElementById("letters_list");
        App.clearList(letters);
    },
    createLetters: function (title) {
        App.shuffled_title = title;
        for (var i = App.shuffled_title.length - 1; i >= 0; i--) {
            var node = document.createElement("li");
            var text = document.createTextNode(App.shuffled_title[i]);
    ...
    selectLetter: function (letter) {
        ...
        App.updateResult(letter.target.id, letter.target.innerText);
    },
    updateResult: function (id, text) {
        ...
        App.current_title.push(text);
        App.checkResult();
    },
    undoLetter: function (letter) {
        App.current_title.pop();
    ...
    checkResult: function () {
        ...
        if (App.compareArrays(App.current_title, App.title)) {
